...
/**
 * The look of a model debug overlay (see {@link ValueIKDebug}), edited through
 * the debug toggle's context menu: the overlay's own switch, whether it draws
 * through the model, dashed chain lines, a global opacity, and an ordered list
 * of its elements. The chain wires and joint dots are shared here but their
 * defaults are supplied per subclass; the accent markers come from the
 * subclasses. The lines element's size is the thickness — zero means hairlines.
 */
